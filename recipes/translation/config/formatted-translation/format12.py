...
class Format12Checker(FormatChecker):
    ...
    def check(self, input_text: str, output_text: str) -> bool:
        ...
        try:
            ...
            logger.debug("✓ All format12 checks passed!")
            return True
    ...
    def _check_translation_constraints(self, input_data: dict, output_data: dict) -> bool:
        ...
        # Check generation field (should be split into reasoning and answer)
        if "generation" in input_data:
            ...
            if not self._check_generation_split(input_data["generation"], output_data["generation"]):
                return False

        # Check expected_answer field (simple translation)
        if "expected_answer" in input_data:
            ...
            if not self._check_simple_translation(
                input_data["expected_answer"], output_data["expected_answer"], "expected_answer"
            ):
                return False

        # Check problem_metadata field (new synthesized field)
        logger.debug("    Checking field 'problem_metadata'...")
        if "problem_metadata" not in output_data:
            logger.debug("    FAILED: Field 'problem_metadata' missing in output")
            return False

        if not self._check_problem_metadata(input_data, output_data["problem_metadata"]):
            return False
    ...
    def _check_problem_metadata(self, input_data: dict, problem_metadata: str) -> bool:
        """Check that problem_metadata is properly created from metadata fields."""
        # Check that it's a non-empty string
        if not isinstance(problem_metadata, str) or not problem_metadata.strip():
            logger.debug("      FAILED: problem_metadata is not a non-empty string")
            return False

        # The problem_metadata should be a descriptive sentence in the target language
        # that synthesizes information from difficulty, topic, and subtopic fields.
        # We keep this check minimal since the exact format and length will vary by language.

        logger.debug(f"      ✓ problem_metadata present and valid: '{problem_metadata[:100]}...'")
        return True

    def _check_generation_split(self, input_value: str, output_value: dict) -> bool:
        """Check that generation field is properly split into reasoning and answer translations."""
        # Check that output is a dict with required keys
        if not isinstance(output_value, dict):
            logger.debug("      FAILED: Generation field is not a dict in output")
            return False

        if "reasoning_translation" not in output_value or "answer_translation" not in output_value:
            logger.debug("      FAILED: Generation field missing 'reasoning_translation' or 'answer_translation' keys")
            return False

        # Extract reasoning (inside think tags) and answer (outside think tags) from input
        think_pattern = r"<think>(.*?)</think>"
        think_match = re.search(think_pattern, input_value, re.DOTALL)

        if think_match:
            input_reasoning = think_match.group(1).strip()
            # Remove the think block from input to get the answer part
            input_answer = re.sub(think_pattern, "", input_value, flags=re.DOTALL).strip()
        else:
            # No think tags found, all content is answer
            input_reasoning = ""
            input_answer = input_value

        # Check LaTeX preservation in both parts
        if input_reasoning:
            if not self._check_latex_preservation(
                input_reasoning, output_value["reasoning_translation"], "generation.reasoning_translation"
            ):
                logger.debug("      FAILED: LaTeX preservation failed for reasoning_translation")
                return False

        if input_answer:
            if not self._check_latex_preservation(
                input_answer, output_value["answer_translation"], "generation.answer_translation"
            ):
                logger.debug("      FAILED: LaTeX preservation failed for answer_translation")
                return False

        logger.debug("      ✓ Generation field properly split and validated")
        return True
