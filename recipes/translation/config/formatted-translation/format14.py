...
class Format14Checker(FormatChecker):
    ...
    def check(self, input_text: str, output_text: str) -> bool:
        ...
        try:
            ...
            logger.debug("✓ All format14 checks passed!")
            return True
    ...
    def _check_translation_constraints(self, input_data: dict, output_data: dict) -> bool:
        ...
        # Check problem field (simple translation)
        if "problem" in input_data:
            if "problem" not in output_data:
                logger.debug("    FAILED: Field 'problem' missing in output")
                return False

            logger.debug("    Checking field 'problem'...")
            if not self._check_simple_translation(input_data["problem"], output_data["problem"], "problem"):
                return False

        # Check generation field (only translate content inside think tags)
        if "generation" in input_data:
            if "generation" not in output_data:
                logger.debug("    FAILED: Field 'generation' missing in output")
                return False

            logger.debug("    Checking field 'generation'...")
            if not self._check_generation_partial(input_data["generation"], output_data["generation"]):
                return False

        # Check expected_answer field (no translation, should be identical)
        if "expected_answer" in input_data:
            if "expected_answer" not in output_data:
                logger.debug("    FAILED: Field 'expected_answer' missing in output")
                return False

            logger.debug("    Checking field 'expected_answer'...")
            if input_data["expected_answer"] != output_data["expected_answer"]:
                logger.debug("    FAILED: Field 'expected_answer' was modified (should be unchanged)")
                return False
            logger.debug("    ✓ Field 'expected_answer' unchanged as expected")

        return True

    def _check_simple_translation(self, input_value: str, output_value: str, field_name: str) -> bool:
        """Check simple translation constraints."""
        # Check that LaTeX formatting is preserved
        if not self._check_latex_preservation(input_value, output_value, field_name):
            logger.debug(f"    FAILED: LaTeX preservation failed for field '{field_name}'")
            return False

        # Check that whitespace and newlines are preserved
        if not self._check_whitespace_preservation(input_value, output_value, field_name):
            logger.debug(f"    FAILED: Whitespace preservation failed for field '{field_name}'")
            return False

        logger.debug(f"    ✓ Field '{field_name}' validation passed")
        return True

    def _check_generation_partial(self, input_value: str, output_value: str) -> bool:
        """Check that only content inside think tags is translated."""
        # Extract content inside and outside think tags from input
        think_pattern = r"<think>(.*?)</think>"
        think_matches = list(re.finditer(think_pattern, input_value, re.DOTALL))

        if not think_matches:
            # No think tags in input, output should be identical to input
            if input_value != output_value:
                logger.debug("      FAILED: No think tags found, but output differs from input")
                return False
            logger.debug("      ✓ No think tags found, output correctly unchanged")
            return True

        # Extract content inside and outside think tags from output
        output_think_matches = list(re.finditer(think_pattern, output_value, re.DOTALL))

        if len(think_matches) != len(output_think_matches):
            logger.debug(
                f"      FAILED: Number of think tags differs - input: {len(think_matches)}, output: {len(output_think_matches)}"
            )
            return False

        # Check that content outside think tags is unchanged
        # Build the parts that should be unchanged
        input_parts_outside_think = []
        output_parts_outside_think = []

        # Get text before first think tag
        first_match = think_matches[0]
        input_parts_outside_think.append(input_value[: first_match.start()])
        output_parts_outside_think.append(output_value[: output_think_matches[0].start()])

        # Get text between think tags
        for i in range(len(think_matches) - 1):
            input_parts_outside_think.append(input_value[think_matches[i].end() : think_matches[i + 1].start()])
            output_parts_outside_think.append(
                output_value[output_think_matches[i].end() : output_think_matches[i + 1].start()]
            )

        # Get text after last think tag
        last_match = think_matches[-1]
        input_parts_outside_think.append(input_value[last_match.end() :])
        output_parts_outside_think.append(output_value[output_think_matches[-1].end() :])

        # Check that all parts outside think tags are unchanged
        for i, (input_part, output_part) in enumerate(zip(input_parts_outside_think, output_parts_outside_think)):
            if input_part != output_part:
                logger.debug(f"      FAILED: Content outside think tags differs at position {i}")
                logger.debug(f"        Input part: '{input_part[:50]}...'")
                logger.debug(f"        Output part: '{output_part[:50]}...'")
                return False

        logger.debug("      ✓ Content outside think tags unchanged")

        # Check LaTeX preservation in think tag content
        for i, (input_match, output_match) in enumerate(zip(think_matches, output_think_matches)):
            input_think_content = input_match.group(1)
            output_think_content = output_match.group(1)

            if not self._check_latex_preservation(input_think_content, output_think_content, f"generation.think[{i}]"):
                logger.debug(f"      FAILED: LaTeX preservation failed in think tag {i}")
                return False

        logger.debug("      ✓ Generation field properly validated")
        return True
