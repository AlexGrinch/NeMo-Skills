...
class Format9Checker(FormatChecker):
    """Checker for format 9 (translate everything incl. expected_answer, add glossary)."""

    def check(self, input_text: str, output_text: str) -> bool:
        logger.debug("Starting format9 check...")
        try:
    ...
    def _check_translation_constraints(self, input_data: dict, output_data: dict) -> bool:
        ...
        # expected_answer must be translated (if there is translatable content)
        if self._has_translatable_text(input_data["expected_answer"]):
            if input_data["expected_answer"] == output_data["expected_answer"]:
                logger.debug("FAILED: expected_answer not translated")
                return False

        # Think tags retained and translated (count match, non-empty if existed)
        if not self._check_think_tags(input_data["output"], output_data["output"]):
            return False

        # Glossary present with >=3 bullet items "- source: target"
        if not self._check_glossary(output_data["output"]):
            return False
    ...
    def _check_glossary(self, output_text: str) -> bool:
        glossary_match = re.search(r"Glossary:\n(.+)", output_text, re.DOTALL)
        if not glossary_match:
            logger.debug("FAILED: Glossary section missing")
            return False
        lines = [ln for ln in glossary_match.group(1).splitlines() if ln.strip()]
        bullet_lines = [ln for ln in lines if ln.strip().startswith("-") and ":" in ln]
        if len(bullet_lines) < 3:
            logger.debug("FAILED: Not enough glossary entries")
            return False
        return True

    def _has_translatable_text(self, text: str) -> bool:
        """Detect non-LaTeX alphabetic content that should be translated."""
        # strip LaTeX blocks and braces
