...
class Format4Checker(FormatChecker):
    """Checker for format 4."""

    def check(self, input_text: str, output_text: str) -> bool:
        logger.debug("Starting format4 check...")
    ...
    def _check_conversation_entry(self, input_conv: dict, output_conv: dict) -> bool:
        ...
        for key in ["source", "target", "language"]:
        ...
        if translation["source"] != input_conv["value"]:
            logger.debug("FAILED: translation.source does not match original value")
            return False

        if not translation["language"].strip():
            logger.debug("FAILED: language field is empty")
            return False

        if not self._check_translation_constraints(input_conv["value"], translation["target"]):
            logger.debug("FAILED: Translation constraints not satisfied")
            return False

        return True

    def _check_translation_constraints(self, original: str, target: str) -> bool:
        # Code blocks must be preserved exactly
        code_pattern = r"```.*?```"
        original_codes = re.findall(code_pattern, original, re.DOTALL)
        target_codes = re.findall(code_pattern, target, re.DOTALL)
        if original_codes != target_codes:
            logger.debug("FAILED: Code blocks not preserved")
            return False

        # Think tags should be present in the same count (content may differ)
        think_pattern = r"<think>(.*?)</think>"
        original_thinks = re.findall(think_pattern, original, re.DOTALL)
        target_thinks = re.findall(think_pattern, target, re.DOTALL)
        if len(original_thinks) != len(target_thinks):
