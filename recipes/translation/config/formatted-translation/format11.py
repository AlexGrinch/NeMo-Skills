...
class Format11Checker(FormatChecker):
    ...
    def _check_translation_constraints(self, input_data: dict, output_data: dict) -> bool:
        ...
        for field in translatable_fields:
            ...
            # Check that output is a dict with "source" and "target" keys
            if not isinstance(output_value, dict):
                logger.debug(f"    FAILED: Field '{field}' is not a dict in output")
                return False

            if "source" not in output_value or "target" not in output_value:
                logger.debug(f"    FAILED: Field '{field}' missing 'source' or 'target' keys")
                return False

            # Check that source matches the original input
            if output_value["source"] != input_value:
                logger.debug(f"    FAILED: Field '{field}' source doesn't match input")
                return False

            # Check that LaTeX formatting is preserved in target
            if not self._check_latex_preservation(input_value, output_value["target"], field):
                logger.debug(f"    FAILED: LaTeX preservation failed for field '{field}'")
                return False

            # Check that whitespace and newlines are preserved in target
            if not self._check_whitespace_preservation(input_value, output_value["target"], field):
    ...
    def _check_whitespace_preservation(self, original: str, translation: str, field_name: str) -> bool:
        ...
        # (allowing for some flexibility in translation while preserving structure)
    ...
    def _extract_json_from_prompt(self, prompt_text: str) -> str:
        ...
        # Format11 typically has the JSON after "Here is the source json object:"
