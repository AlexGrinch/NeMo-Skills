...
class Format0Checker(FormatChecker):
    ...
    def _check_translation_constraints(self, original: str, translation: str) -> bool:
        ...
        # Extract code blocks - should not be translated
        code_pattern = r"```python(.*?)```"
        original_codes = re.findall(code_pattern, original, re.DOTALL)
        translation_codes = re.findall(code_pattern, translation, re.DOTALL)

        # Code blocks should be identical
        if original_codes != translation_codes:
            logger.debug(
                f"      FAILED: Code block content differs - original: {original_codes}, translation: {translation_codes}"
            )
            return False
        logger.debug(f"      ✓ Code blocks preserved ({len(original_codes)} found)")

        # Remove think tags and code blocks to check if other text was translated
