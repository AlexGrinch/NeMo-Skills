...
class Format11Checker(FormatChecker):
    ...
    def _check_translation_constraints(self, input_data: dict, output_data: dict) -> bool:
        ...
        for field in translatable_fields:
            ...
            if not self._check_latex_preservation(input_value, output_value, field):
                logger.debug(f"    FAILED: LaTeX preservation failed for field '{field}'")
                return False

            # Check that whitespace and newlines are preserved in structure
            if not self._check_whitespace_preservation(input_value, output_value, field):
